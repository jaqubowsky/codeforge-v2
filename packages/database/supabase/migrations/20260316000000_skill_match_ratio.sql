-- Change skill matching from absolute count to percentage-based
--
-- Before: min_skill_matches = 1 (at least 1 skill must match)
-- After:  min_skill_ratio = 0.3 (at least 30% of offer's skills must match)
--
-- This reduces noise from offers where the user matches only 1 out of 10 required skills.

DROP FUNCTION IF EXISTS public.match_jobs_for_user(extensions.vector, text[], text[], text[], text[], double precision, integer, integer);
...
BEGIN
  ...
  skill_filtered AS (
    ...
    WHERE sc.total_techs = 0
       OR sc.matching_techs >= CEIL(sc.total_techs * min_skill_ratio)
  )
  SELECT
    sf.id AS offer_id,
...
3. At least 30% of offer skills must match user skills (using normalization)
